...
* **RNF02 - Disponibilidade e Confiabilidade:** O sistema deve garantir uma alta disponibilidade (uptime de 99,9%), possuindo infraestrutura escalável (como servidores em nuvem) para evitar quedas e lentidão durante os períodos críticos, como semanas de provas, fechamento de notas e rematrículas[cite: 29].
* **RNF03 - Performance e Predição:** O sistema deve ser capaz de processar os cruzamentos de dados (frequência, notas, engajamento no AVA) em segundo plano, garantindo que a emissão de alertas na interface do professor (durante a chamada) ocorra em tempo real, sem travamentos[cite: 30].
